...
def get_skl_metrics(labels, predictions, num_labels):
    # 得到混淆矩阵和，在这里我们需要将生成的混淆矩阵转换成tensor
    labels = labels.eval()
    predictions = predictions.eval()
    num_labels = num_labels.eval()
    sm, = confusion_matrix(labels, predictions, num_labels)
    print(sm)
